...
function M.expand(opts)
  ...
  local cur_item_user_data
  if cur_item then
    cur_item_user_data = util.get_user_data(cur_item)
  else
    cur_item_user_data = {}
  end

  if opts.add_to_existing then
    num_before = num_before + (cur_item_user_data.num_before or 0)
  end
  num_before = math.max(0, num_before)
  cur_item.num_before = num_before
  local num_after = opts.after or 2
  if opts.add_to_existing then
    num_after = num_after + (cur_item_user_data.num_after or 0)
  end
  num_after = math.max(0, num_after)
  cur_item_user_data.num_after = num_after
  cur_item_user_data.num_before = num_before
  cur_item.user_data = cur_item_user_data
...
function M.collapse(opts)
  ...
  for i, item in ipairs(qf_list.items) do
    local user_data = util.get_user_data(item)
    user_data.num_before = 0
    user_data.num_after = 0
